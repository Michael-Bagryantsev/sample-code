...
class MessagesHelper
{
     ...
     * image replace based on https://stackoverflow.com/a/43705283
     */
    public static function formatMessageText($text)
    {
        ...
        $text = self::decode_entities_full($text);
        $text = preg_replace('#INSERTEDIMAGE#ms', "&#8205;", $text);
        $text = strip_tags($text, '<b><strong><i><em><a><code><pre>');
    ...
    public static function decode_entities_full($string, $quotes = ENT_COMPAT, $charset = 'UTF-8') {
        return html_entity_decode(preg_replace_callback('/&([a-zA-Z][a-zA-Z0-9]+);/', 'self::convert_entity', $string), $quotes, $charset);
    }
